...
use super::{
    ArtifactDisposition, DeployComponent, DeployProgress,
    config::{
...
enum BackendPreparation {
    Configured(PathBuf),
    Release { target: &'static str },
}

enum DashboardPreparation {
    Configured(PathBuf),
    Node { node: OsString, cached: bool },
}
...
impl<'a> ArtifactInstaller<'a> {
    ...
    pub(super) fn ensure(
        ...
        progress(DeployProgress::CheckingDependencies);
        let (backend, dashboard) = self.check_dependencies()?;
        progress(DeployProgress::DependenciesReady);
        Ok(RuntimeArtifacts {
            backend: self.ensure_backend(backend, progress)?,
            dashboard: self.ensure_dashboard(dashboard, progress)?,
        })
    }

    fn check_dependencies(&self) -> Result<(BackendPreparation, DashboardPreparation), CliError> {
        let backend = if let Some(path) = std::env::var_os(BACKEND_BIN_ENV).map(PathBuf::from) {
            validate_executable(&path, "ABYSS_LOCAL_BACKEND_BIN")?;
            BackendPreparation::Configured(path)
        } else {
            BackendPreparation::Release {
                target: backend_target()?,
            }
        };
        let dashboard = if let Some(path) = std::env::var_os(DASHBOARD_BIN_ENV).map(PathBuf::from) {
            validate_executable(&path, "ABYSS_LOCAL_DASHBOARD_BIN")?;
            DashboardPreparation::Configured(path)
        } else {
            let node = require_command_version("node", MINIMUM_NODE_MAJOR)?;
            let cached = installed_dashboard_is_valid(&self.paths.dashboard_runtime_dir())?;
            if !cached {
                let _npm = require_command_version("npm", MINIMUM_NPM_MAJOR)?;
            }
            DashboardPreparation::Node { node, cached }
        };
        Ok((backend, dashboard))
    }
    ...
    fn ensure_backend(
        ...
        progress(DeployProgress::PreparingArtifact(DeployComponent::Backend));
        let target = match preparation {
            BackendPreparation::Configured(path) => {
                progress(DeployProgress::ArtifactReady {
                    component: DeployComponent::Backend,
                    disposition: ArtifactDisposition::Configured,
                });
                return Ok(path);
            }
            BackendPreparation::Release { target } => target,
        };
        ...
        progress(DeployProgress::VerifyingBackend);
    ...
    fn ensure_dashboard(
        &self,
        preparation: DashboardPreparation,
        progress: &mut dyn FnMut(DeployProgress),
    ) -> Result<DashboardArtifact, CliError> {
        progress(DeployProgress::PreparingArtifact(
            DeployComponent::Dashboard,
        ));
        let (node, cached) = match preparation {
            DashboardPreparation::Configured(path) => {
                progress(DeployProgress::ArtifactReady {
            ...
            DashboardPreparation::Node { node, cached } => (node, cached),
        };
        ...
        let result = (|| {
            progress(DeployProgress::InstallingDashboard);
    ...
    fn download_backend(
        ...
        validate_declared_length(total, url, maximum_bytes)?;
        progress(DeployProgress::DownloadingBackend {
            downloaded: 0,
            total,
        });
        read_bounded(response, url, maximum_bytes, |downloaded| {
            progress(DeployProgress::DownloadingBackend { downloaded, total });
        })
    }
}
